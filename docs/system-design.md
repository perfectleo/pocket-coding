...
- **嵌套环境清洗**：若 `server` 本身是在某个 AI CLI 会话内启动的（本项目即用 claude/codebuddy
  开发），父进程会导出 `CLAUDECODE=1` / `CLAUDE_CODE_ENTRYPOINT` / `CLAUDE_CODE_SSE_PORT`
  等"嵌套标记"。这些若随 `process.env` 泄漏进 PTY，新拉起的 `claude` 会判定自己嵌套在另一个
  agent 里并**禁用 `/model` `/clear` 等交互命令**（报 "isn't available in this environment"）。
  `TerminalManager.open` 用 `sanitizeTerminalEnv` 剥掉这些标记（保留 `ANTHROPIC_*` 鉴权变量），
  使终端里的 CLI 以干净的顶层交互会话启动。
