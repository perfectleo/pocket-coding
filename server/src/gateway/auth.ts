...
export function newSessionId(): string {
  // Pocket's internal session ID (DB primary key). The AI tool's own
  // session ID is generated by the tool itself (claude/codebuddy emit
  // session_id in every message; codex emits thread_id in thread.started)
  // and captured from stdout into Session.externalSessionId. We do NOT
  // pre-assign --session-id — the tool is the source of truth for its
  // own session, and we resume with --resume <captured-id>.
