...
export async function buildHttpServer(): Promise<FastifyInstance> {
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    if ((req.routerPath || '').startsWith('/preview/')) return;
  ...
  app.get('/api/roots', async () => {
    return { root: config.workspacesDir };
  });

  app.get('/api/roots/browse', async (req, reply) => {
    const rel = (req.query as { path?: string } | undefined)?.path ?? '';
    const root = config.workspacesDir;
    const abs = safeJoin(root, rel);
    if (!abs) return reply.code(400).send({ error: 'invalid_path' });
    try {
      const stats = statSync(abs);
      if (!stats.isDirectory()) return reply.code(400).send({ error: 'not_a_directory' });
      const entries = readdirSync(abs, { withFileTypes: true })
        .filter((e) => !e.name.startsWith('.') && e.name !== 'node_modules')
        .filter((e) => e.isDirectory())
        .map((e) => ({ name: e.name, path: join(rel, e.name), dir: true }))
        .sort((a, b) => a.name.localeCompare(b.name));
      return { entries, root, path: rel };
    } catch (err) {
      return reply.code(500).send({ error: 'browse_failed', message: (err as Error).message });
    }
  });
  ...
  app.post('/api/sessions', async (req, reply) => {
    ...
    // Process is spawned lazily on first input — no eager start.
    return {
      id: session.id,
  ...
  app.delete('/api/sessions/:id', async (req, reply) => {
    ...
    if (session) {
      sessionManager.deleteSession(session, store);
      return { ok: true };
    }
    // Session not in memory — try DB row (server may have restarted).
    const row = store.getSession(id);
    if (!row) return reply.code(404).send({ error: 'not_found' });
    store.deleteSessionCascade(id);
    store.audit(null, 'session_delete', id);
    return { ok: true };
  });
  ...
  app.get('/api/sessions/:id/diff/:cpId?', async (req, reply) => {
    ...
    const files = (req.query as { files?: string } | undefined)?.files;
    const fileList = files ? files.split(',').filter(Boolean) : undefined;
    const s = sessionManager.get(id);
    if (!s) return reply.code(404).send({ error: 'session_not_in_memory' });
    try {
      const diff = await checkpoint.computeDiff(s.cwd, store, id, cpId, fileList);
      return { diff };
    } catch (err) {
      return reply.code(500).send({ error: 'diff_failed', message: (err as Error).message });
    }
  });

  app.post('/api/sessions/:id/accept/:cpId', async (req, reply) => {
    const { id, cpId } = req.params as { id: string; cpId: string };
    const body = (req.body as { files?: string[] } | null) ?? {};
    const s = sessionManager.get(id);
    if (!s) return reply.code(404).send({ error: 'session_not_in_memory' });
    try {
      ...
      return reply.code(400).send({ error: 'accept_failed', message: (err as Error).message });
    }
  });
  ...
  app.post('/api/sessions/:id/rollback/:cpId', async (req, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({ error: 'rollback_failed', message: (err as Error).message });
    }
  });
  ...
  app.post('/api/sessions/:id/approve/:callId', async (req, reply) => {
    ...
    if (!ok) return reply.code(404).send({ error: 'no_pending_approval' });
  ...
  app.get('/api/sessions/:id/files', async (req, reply) => {
    ...
    try {
      ...
      return reply.code(500).send({ error: 'files_failed', message: (err as Error).message });
    }
  });
  ...
  app.get('/api/sessions/:id/files/content', async (req, reply) => {
    ...
    try {
      const stats = statSync(abs);
      if (stats.isDirectory()) return reply.code(400).send({ error: 'is_directory' });
      if (stats.size > 512 * 1024) return reply.code(413).send({ error: 'file_too_large' });
      const buf = readFileSync(abs);
      return { content: buf.toString('utf8'), size: stats.size, path: rel };
    } catch (err) {
      return reply.code(500).send({ error: 'read_failed', message: (err as Error).message });
    }
  });
  ...
  app.post('/api/sessions/:id/preview/start', async (req, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({ error: 'preview_start_failed', message: (err as Error).message });
    }
  });
  ...
  app.post('/api/sessions/:id/preview/stop', async (req) => {
    ...
    if (!h) return { ok: false, error: 'no_preview' };
  ...
  app.get('/api/sessions/:id/preview/status', async (req) => {
    const { id } = req.params as { id: string };
    const h = previewManager.getBySession(id);
    if (!h) return { state: 'stopped', token: null, port: null };
    return {
      state: h.state,
      token: h.token,
      port: h.port,
  ...
  app.post('/api/devices/push/register', async (req, reply) => {
    const parsed = registerPushSchema.safeParse(req.body);
    if (!parsed.success) return reply.code(400).send({ error: 'bad_request' });
    const deviceId = (req as unknown as { deviceId: string }).deviceId;
    store.registerPushToken(deviceId, parsed.data.platform, parsed.data.token);
    store.audit(null, 'push_register', deviceId, JSON.stringify({ platform: parsed.data.platform }));
    return { ok: true };
  });

  app.post('/api/devices/push/unregister', async (req, reply) => {
    const body = (req.body as { token?: string } | null) ?? {};
    if (!body.token || body.token.length < 16) return reply.code(400).send({ error: 'bad_request' });
    const deviceId = (req as unknown as { deviceId: string }).deviceId;
    store.unregisterPushToken(deviceId, body.token);
    store.audit(null, 'push_unregister', deviceId);
    return { ok: true };
  });
...
function safeJoin(cwd: string, rel: string): string | null {
  ...
  if (rel0.startsWith('..') || isAbsolute(rel0)) return null;
  return abs;
}
