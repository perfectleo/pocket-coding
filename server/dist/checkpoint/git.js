...
function shadowDir(cwd) {
    return join(cwd, '.pocket', 'shadow.git');
}
function baseArgs(cwd) {
    return [
...
async function git(cwd, sub, opts = {}) {
...
export function shadowExists(cwd) {
    return existsSync(shadowDir(cwd));
}
export async function initShadow(cwd) {
    if (shadowExists(cwd))
        return;
...
export async function headRef(cwd) {
...
export async function diffNameOnly(cwd, base, head) {
    const args = head ? ['diff', '--name-only', base, head, '--', ':!/.pocket'] : ['diff', '--name-only', base, '--', ':!/.pocket'];
    const out = await git(cwd, args);
    return out.split('\n').map((s) => s.trim()).filter(Boolean);
}
export async function diffRaw(cwd, base, files) {
...
export async function restoreFromCommit(cwd, commit, files) {
    if (files && files.length > 0) {
...
export async function listCommits(cwd, limit = 50) {
    const out = await git(cwd, ['log', `--max-count=${limit}`, '--format=%H']);
    return out.split('\n').map((s) => s.trim()).filter(Boolean);
}
export async function pruneOlderThan(cwd, keepRef) {
