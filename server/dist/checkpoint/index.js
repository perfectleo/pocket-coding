...
import { commitAll, diffNameOnly, diffRaw, headRef, initShadow, restoreFromCommit, shadowExists, } from './git.js';
const execFileP = promisify(execFile);
export async function ensureShadow(cwd, store, sessionId) {
    if (!shadowExists(cwd)) {
...
export async function snapshot(cwd, store, sessionId, turnId) {
    ...
    const files = await Promise.all(changedPaths.map(async (p) => {
...
export async function computeDiff(cwd, store, sessionId, cpId, files) {
    await ensureShadow(cwd, store, sessionId);
    const baseline = store.getSession(sessionId)?.baseline_ref ?? (await headRef(cwd));
    let base = baseline;
    if (cpId) {
...
export async function accept(cwd, store, sessionId, cpId, files) {
    const cp = store.getCheckpoint(cpId);
    if (!cp)
        throw new Error('checkpoint_not_found');
    if (cp.session_id !== sessionId)
...
export async function rollback(cwd, store, sessionId, cpId) {
    const cp = store.getCheckpoint(cpId);
    if (!cp)
        throw new Error('checkpoint_not_found');
    if (cp.session_id !== sessionId)
...
async function fileStat(cwd, base, head, file) {
    // Use --numstat to get added/removed counts per file.
    const { stdout } = await execFileP('git', [`--git-dir=${cwd}/.pocket/shadow.git`, 'diff', '--numstat', base, head, '--', file], { maxBuffer: 8 * 1024 * 1024 }).catch(() => ({ stdout: '' }));
